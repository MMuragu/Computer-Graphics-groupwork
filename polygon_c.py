...
def isPointInsidePolygon(x, y):
    """Check if point is inside polygon"""
    inside = False
    n = NUM_VERTICES
    for i in range(n):
        j = (i - 1) % n
        xi, yi = polygonVertices[i][0], polygonVertices[i][1]
        xj, yj = polygonVertices[j][0], polygonVertices[j][1]
        
        if ((yi > y) != (yj > y)) and (x < (xj - xi) * (y - yi) / (yj - yi) + xi):
            inside = not inside
    return inside

def generateInteriorPoints():
    """Generate interior points"""
    global interiorPoints
    interiorPoints.clear()
    
    # Find bounding box
    minX = polygonVertices[0][0]
    maxX = polygonVertices[0][0]
    minY = polygonVertices[0][1]
    maxY = polygonVertices[0][1]
...
def drawAsteriskFill():
    """Draw asterisk fill with shades"""
    for p in interiorPoints:
        # Calculate shade based on x position
        shade = 0.5 + (p[0] - 2.0) / 20.0
        if shade > 1.0:
            shade = 1.0
        if shade < 0.4:
            shade = 0.4
        
        glColor3f(shade, 0.0, 0.0)  # Set asterisk color (red shades)
...
def main():
    ...
    glutCreateWindow(b"c): Polygon with Asterisk Fill")  
